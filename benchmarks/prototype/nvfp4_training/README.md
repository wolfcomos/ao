...
recipe (rowwise cast plus columnwise RHT-128 on the activation, RHT-128 on both axes of
the gradient, MS-EDEN on the gradient quantize) and the V1_REQUANT weight path
(`row_cast_quantize` in the forward, then the four `col_*` requantizers that rebuild the
dgrad operand from the packed forward weight instead of re-reading the BF16 weight).
One launch per kernel over the local expert stack at `E = 4`, fed the inputs the recipe
builds: 128-aligned uniform token groups with cumulative row-end offsets for the
token-jagged ops, the `(E, N, K)` weight stack for the expert-uniform ones, and the
packed `row_fp4_w` / `row_sf_w` plus the matching `*_requant_amax` output for the
requantizers. Tokens per expert default to the weight row count, as in
`bench_group_rht_quantize_row_col`.

The bench script that produced this table (`bench_group_v2_kernels.py`, one launch per
kernel with `--kernels / --experts / --tokens-per-expert / --shape-set / --rounding /
--json`) is not checked in with it; the table is the record.

#### Methodology

- Reports **device kernel time** of everything the custom op launches
  (`bench_utils.kernel_time_us`, 15 warmups / 50 iterations). That is op time, not
  kernel-only time: the RHT-128 ops build their rotation matrix in torch
  (`get_dynamic_rht_matrix`, 6-9 us at this clock) and the amax ops zero-fill their
  output (about 2 us) inside the timed region. Compare a port of one of these kernels on
  kernel-only time, or make it build the RHT matrix the same way.
- Bytes are the tensors each op reads and writes; `pct_peak` is against the
  8064 GB/s device-properties peak of this GPU.
- Every kernel was timed inside its own `try/except`, so a compile failure of one op is
  recorded verbatim instead of aborting the pass.
- Run environment: NVIDIA GB200 with the SM clock capped at 1200 MHz (applications
  clock), so absolute times are roughly 1.6x those of the earlier sections in this file;
  ratios are comparable. Two toolchains, each the median of three full passes:
  Triton 3.6.0 (PyTorch 2.14.0a0, CUDA 13.4) and Triton 3.8.0 (PyTorch 2.14.0a0,
  CUDA 13.x (pre-release)). Triton 3.6.0 fails to compile the two RHT-128 amax ops
...
#### Results (E = 4, median of 3 passes)
...
|---|---|---|---|---|---:|---:|---:|---:|
| 16B | gate/up (w1/w3) | row_cast_quantize | w (4, 1408, 2048) | rtne | 11.99 | 11.77 | 2511.0 | 31.14 |
| 16B | gate/up (w1/w3) | col_cast_requant_amax | w (4, 1408, 2048) | - | 13.59 | 11.68 | 555.7 | 6.89 |
| 16B | gate/up (w1/w3) | col_cast_requantize | w (4, 1408, 2048) | rtne | 82.39 | 56.19 | 230.9 | 2.86 |
| 16B | gate/up (w1/w3) | col_rht_requant_amax | w (4, 1408, 2048) | - | 25.93 | 25.82 | 251.3 | 3.12 |
| 16B | gate/up (w1/w3) | col_rht_requantize | w (4, 1408, 2048) | rtne | 27.04 | 28.09 | 462.0 | 5.73 |
| 16B | gate/up (w1/w3) | row_cast_col_rht_amax | x (5632, 2048) | - | n/a | 35.93 | 642.0 | 7.96 |
| 16B | gate/up (w1/w3) | row_cast_col_rht_quantize | x (5632, 2048) | rtne | 33.07 | 30.74 | 1172.7 | 14.54 |
| 16B | gate/up (w1/w3) | row_cast_col_rht_quantize | x (5632, 2048) | rs | 70.35 | 59.60 | 604.8 | 7.50 |
| 16B | gate/up (w1/w3) | row_rht_col_rht_amax | dy (5632, 1408) | - | n/a | 35.59 | 445.6 | 5.53 |
| 16B | gate/up (w1/w3) | row_rht_col_rht_quantize_ms_eden | dy (5632, 1408) | ms_eden | 50.54 | 51.39 | 482.2 | 5.98 |
| 16B | down (w2) | row_cast_quantize | w (4, 2048, 1408) | rtne | 11.96 | 11.71 | 2523.1 | 31.29 |
| 16B | down (w2) | col_cast_requant_amax | w (4, 2048, 1408) | - | 13.69 | 11.65 | 557.1 | 6.91 |
| 16B | down (w2) | col_cast_requantize | w (4, 2048, 1408) | rtne | 82.26 | 56.13 | 231.2 | 2.87 |
| 16B | down (w2) | col_rht_requant_amax | w (4, 2048, 1408) | - | 25.77 | 25.69 | 252.5 | 3.13 |
| 16B | down (w2) | col_rht_requantize | w (4, 2048, 1408) | rtne | 26.91 | 28.10 | 461.8 | 5.73 |
| 16B | down (w2) | row_cast_col_rht_amax | x (8192, 1408) | - | n/a | 35.90 | 642.6 | 7.97 |
| 16B | down (w2) | row_cast_col_rht_quantize | x (8192, 1408) | rtne | 33.03 | 30.67 | 1175.2 | 14.57 |
| 16B | down (w2) | row_cast_col_rht_quantize | x (8192, 1408) | rs | 70.65 | 59.60 | 604.8 | 7.50 |
| 16B | down (w2) | row_rht_col_rht_amax | dy (8192, 2048) | - | n/a | 54.33 | 617.6 | 7.66 |
| 16B | down (w2) | row_rht_col_rht_quantize_ms_eden | dy (8192, 2048) | ms_eden | 84.62 | 86.60 | 605.4 | 7.51 |
| 671B | gate/up (w1/w3) | row_cast_quantize | w (4, 2048, 7168) | rtne | 54.84 | 52.21 | 2881.8 | 35.74 |
| 671B | gate/up (w1/w3) | col_cast_requant_amax | w (4, 2048, 7168) | - | 47.56 | 38.19 | 865.0 | 10.73 |
| 671B | gate/up (w1/w3) | col_cast_requantize | w (4, 2048, 7168) | rtne | 390.59 | 265.34 | 249.0 | 3.09 |
| 671B | gate/up (w1/w3) | col_rht_requant_amax | w (4, 2048, 7168) | - | 77.86 | 79.42 | 415.9 | 5.16 |
| 671B | gate/up (w1/w3) | col_rht_requantize | w (4, 2048, 7168) | rtne | 90.33 | 105.54 | 626.0 | 7.76 |
| 671B | gate/up (w1/w3) | row_cast_col_rht_amax | x (8192, 7168) | - | n/a | 119.12 | 985.9 | 12.23 |
| 671B | gate/up (w1/w3) | row_cast_col_rht_quantize | x (8192, 7168) | rtne | 128.54 | 112.92 | 1625.0 | 20.15 |
| 671B | gate/up (w1/w3) | row_cast_col_rht_quantize | x (8192, 7168) | rs | 286.14 | 246.95 | 743.1 | 9.21 |
| 671B | gate/up (w1/w3) | row_rht_col_rht_amax | dy (8192, 2048) | - | n/a | 54.45 | 616.3 | 7.64 |
| 671B | gate/up (w1/w3) | row_rht_col_rht_quantize_ms_eden | dy (8192, 2048) | ms_eden | 84.60 | 86.66 | 605.0 | 7.50 |
| 671B | down (w2) | row_cast_quantize | w (4, 7168, 2048) | rtne | 54.99 | 52.35 | 2874.3 | 35.64 |
| 671B | down (w2) | col_cast_requant_amax | w (4, 7168, 2048) | - | 47.59 | 38.18 | 865.2 | 10.73 |
| 671B | down (w2) | col_cast_requantize | w (4, 7168, 2048) | rtne | 390.08 | 266.77 | 247.6 | 3.07 |
| 671B | down (w2) | col_rht_requant_amax | w (4, 7168, 2048) | - | 77.92 | 79.40 | 416.0 | 5.16 |
| 671B | down (w2) | col_rht_requantize | w (4, 7168, 2048) | rtne | 90.54 | 106.01 | 623.2 | 7.73 |
| 671B | down (w2) | row_cast_col_rht_amax | x (28672, 2048) | - | n/a | 119.37 | 983.8 | 12.20 |
| 671B | down (w2) | row_cast_col_rht_quantize | x (28672, 2048) | rtne | 127.63 | 112.59 | 1629.9 | 20.21 |
| 671B | down (w2) | row_cast_col_rht_quantize | x (28672, 2048) | rs | 284.94 | 248.34 | 738.9 | 9.16 |
| 671B | down (w2) | row_rht_col_rht_amax | dy (28672, 7168) | - | n/a | 440.12 | 933.9 | 11.58 |
| 671B | down (w2) | row_rht_col_rht_quantize_ms_eden | dy (28672, 7168) | ms_eden | 865.93 | 801.36 | 801.5 | 9.94 |

`row_cast_quantize` is the only one of the nine near bandwidth (31-36% of peak). The
no-transform `col_cast_requantize` runs 3-4x slower than its RHT twin
`col_rht_requantize` at 2-3% of peak: the compiled kernel holds 254 registers and 64 KB
of shared memory and materialises its 128x128 tile three times (once per layout
conversion), so it is neither memory- nor occupancy-bound; that is the largest headroom
of the nine. The RHT-128 activation and gradient kernels sit at 8-20% of peak, with
6-9 us of each op spent building the rotation matrix in torch.
