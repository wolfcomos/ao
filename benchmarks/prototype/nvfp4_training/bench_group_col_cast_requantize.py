...
"""Benchmark the grouped lazy columnwise weight requantize across backends (triton, cutedsl).
...
W_qdq per expert and requantizes its transpose rowwise along the transposed axis
against the group amaxes of ``bench_group_col_cast_requant_amax``'s op: RTNE FP4 codes
and swizzled e4m3 block scales, the V1_REQUANT backward's dgrad weight operand. No sign
vector. Reports device kernel time (see bench_utils.kernel_time_us) for each available
backend on the DeepSeek-V3 shapes, with the cutedsl-vs-triton speedup.
...
def make_runner(
    ...
    if backend == "triton":
        ...
        from torchao.prototype.moe_training.nvfp4_training.group_col_cast_requantize_triton import (
            triton_group_col_cast_requantize as op,
        )
        ...
        from torchao.prototype.moe_training.nvfp4_training.group_col_cast_requantize_cutedsl import (
            cutedsl_group_col_cast_requantize as op,
        )
    else:
        raise ValueError(f"unknown backend {backend}")

    return lambda: op(row_fp4_w, row_sf_w, global_amax, amax_w_qdq_t, num_tensors)
...
def run_experiment(config: ExperimentConfig) -> Optional[ExperimentResult]:
    ...
    # The rowwise codes and scales and the group amaxes are inputs, computed once outside
    # the timed region; the two backends' row-cast and requant amax ops are bitwise
    # identical, so either feeds both requantize ops.
    if cutedsl_nvfp4_kernels_available():
        from torchao.prototype.moe_training.nvfp4_training.group_col_cast_requantize_cutedsl import (
            cutedsl_group_col_cast_requant_amax as amax_op,
        )
        ...
        from torchao.prototype.moe_training.nvfp4_training.group_col_cast_requantize_triton import (
            triton_group_col_cast_requant_amax as amax_op,
        )
    ...
    amax_w_qdq_t = amax_op(row_fp4_w, row_sf_w, global_amax, E)

    us: Dict[str, float] = {}
    for backend in BACKENDS:
        runner = make_runner(backend, row_fp4_w, row_sf_w, global_amax, amax_w_qdq_t, E)
