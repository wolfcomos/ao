...
"""Benchmark the grouped rotated columnwise weight requant amax across backends (triton, cutedsl).
...
transform, the dgrad signs) and returns the per-expert amax of |W_qdq^T @ R_n|: the
amax the V2 backward's dgrad weight operand is requantized against. Reports device
kernel time (see bench_utils.kernel_time_us) for each available backend on the
...
def make_runner(
    ...
    if backend == "triton":
        ...
        from torchao.prototype.moe_training.nvfp4_training.group_col_rht_requantize_triton import (
            triton_group_col_rht_requant_amax as op,
        )
        ...
        from torchao.prototype.moe_training.nvfp4_training.group_col_rht_requantize_cutedsl import (
            cutedsl_group_col_rht_requant_amax as op,
        )
    else:
        raise ValueError(f"unknown backend {backend}")

    return lambda: op(row_fp4_w, row_sf_w, global_amax, dgrad_rht, num_tensors)
...
def run_experiment(config: ExperimentConfig) -> Optional[ExperimentResult]:
    ...
    for backend in BACKENDS:
        runner = make_runner(backend, row_fp4_w, row_sf_w, global_amax, dgrad_rht, E)
