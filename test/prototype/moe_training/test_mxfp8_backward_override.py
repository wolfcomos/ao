...
if not _SM100_KERNELS_AVAILABLE:
    pytest.skip(
        "SM100 MXFP8 kernels (CuTe DSL + Triton) unavailable",
        allow_module_level=True,
    )
...
def _reference_grads(A, B, offs, grad_output):
    ...
    ref_grad_A = torch._grouped_mm(
        grad_output, B, offs=offs, out_dtype=torch.bfloat16
    )
...
def test_dequantized_backward_close_to_reference_and_distinct():
    ...
    # error per operand, ~5% max relative error at these shapes.
...
def test_noncontiguous_grad_output_wgrad():
    """The Triton dim1 cast in the quantized wgrad asserts contiguity;
    ...
    out = _to_mxfp8_then_scaled_grouped_mm(
        A_leaf, B_leaf.transpose(-2, -1), offs=offs
    )
    out.sum().backward()
