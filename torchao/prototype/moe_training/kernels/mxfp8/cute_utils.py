...
if _cutedsl_runtime_available():
    ...
    # 4.6.0 dropped it and infers the type instead. The API break sits on the
    # 4.6 minor boundary, so compare major/minor only — parse_version maps
    # prerelease/local suffixes ("4.6.0.dev0", "+git...") to patch -1, which a
    # full triple compare would misread as pre-4.6. The runtime probe checks
    # importability, not distribution names, so the version lookup can miss
    # (e.g. cutlass vendored under another distribution, or a rebuilt wheel
    # with a nonstandard version string); assume the current API in those
    # cases rather than failing the whole package import.
    try:
        _CUTLASS_DSL_BEFORE_4_6_0 = (
            parse_version(importlib.metadata.version("nvidia-cutlass-dsl"))[:2]
            < parse_version("4.6.0")[:2]
        )
