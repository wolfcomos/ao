...
The arithmetic is transcribed operation for operation from TransformerEngine's
``quantize_4over6_nvfp4.cuh`` so codes and scales are bitwise identical to
that recipe (see ``compute_scale_pair`` and ``accumulate_dequant_error``
...
def four_over_six_global_encode_scale(
    ...
    """``compute_global_encode_scaling_factor_FP4<E4M3_MAX>``: bound * 6 / amax.
...
def _candidate_error(
    ...
    """``accumulate_dequant_error``: FP32 adds in element order, per 1x16 group.
    ...
    # correctly-rounded FP32 division, matching the kernel's __fdiv_rn.
...
def four_over_six_quantize(
    ...
    *,
    ...
    # arithmetic below with bitwise-identical codes and scales; ineligible
    # shapes/dtypes silently fall through to the pure-PyTorch body.
    ...
    # compute_scale_pair: base = (block_amax / 6) * S_enc, then the 1.5x
