...
pure-PyTorch body (itself transcribed from TransformerEngine's
``quantize_4over6_nvfp4.cuh``), producing bitwise-identical codes and scales.
...
Eligibility (the dispatch gate in ``four_over_six_quantize``): CUDA bf16 or
fp32 input, contiguous, C % 64 == 0 (the tile width; also makes the u32 scale
store aligned). Ineligible calls fall through to the pure-PyTorch body.
"""
...
from .hadamard_cutedsl_utils import (
    ...
    raise_if_cutedsl_nvfp4_unavailable,
)
...
@functools.cache
def _cutedsl_quantize_available() -> bool:
    """Cached ``cutedsl_nvfp4_kernels_available()`` for the per-call dispatch gate."""
    return cutedsl_nvfp4_kernels_available()
...
def _cutedsl_quantize_eligible(x: torch.Tensor) -> bool:
    ...
    return (
        ...
        and x.is_contiguous()
        and _cutedsl_quantize_available()
    )
...
def _compile_four_over_six_quantize_cutedsl(
    ...
    def _fmax_f32(
        ...
        # all-NaN group yields amax 0 (the TE amax semantics). This is NOT the
        # NaN-propagating _max_f32 the RHT kernels use.
        return cutlass.Float32(
            llvm.inline_asm(
                T.f32(),
                [a.ir_value(loc=loc, ip=ip), b.ir_value(loc=loc, ip=ip)],
    ...
    def _tile_reduce_sum16(v):
        """The exact TE error tree: value(l) += value(l+8), +=(l+4), +=(l+2),
...
@torch.library.custom_op("torchao::four_over_six_quantize_cutedsl", mutates_args=())
def four_over_six_quantize_cutedsl(
    ...
    with one documented exception: NaN inputs follow the TE kernel semantics
    (NaN-dropping block amax, NaN -> +6 FP4 codes) while torch's ``amax``
    ...
    """
    raise_if_cutedsl_nvfp4_unavailable("four_over_six_quantize_cutedsl")
    if x.ndim != 2:
